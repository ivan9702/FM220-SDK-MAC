...
#define W 176
    vga_getframe(handle, rawImage, W * W, 1);
    write_bmp("/tmp/raw.bmp", rawImage, rawImage, rawImage, W, W);
    
    uint8_t *single = (uint8_t *)calloc(W * W, sizeof(uint8_t));
    
    for (int i = 0; i < W; i++) {
        for (int j = 0; j < W / 2; j++) {
            uint8_t px = rawImage[(i * 1 + 0) * W + (j * 2 + 0)];
            printf("%02x ", px);
            single[i * (W / 2) + j] = px;
        }
        puts("");
    }
    write_bmp("/tmp/single.bmp", single, single, single, W/2, W);
